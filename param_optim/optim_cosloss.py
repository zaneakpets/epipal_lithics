    ...
    try:
        ...
        for kk in range(MAX_ITR):
            ...
            if config.model.loss == 'cosface' or config.model.loss == 'softmax' :
                train_generator = CosFaceGenerator(config, datagen_args, is_train=True, is_random_transform=True)
                train_generator_testing = CosFaceGenerator(config, datagen_args, is_train=True, is_random_transform=False)
                valid_generator = CosFaceGenerator(config, datagen_args, is_train=False, is_random_transform=False)
            ...
            trainer = CosLossModelTrainer(model.model, train_generator, valid_generator, train_generator_testing,
                                          config, train_generator.class_list)
            ...
            print(trainer.val_acc[-1])
            accuracy = trainer.get_accuracy(0)

            lines[int(experiment) + 1][1] = '{0:.3f}'.format(trainer.val_acc[-1])
            lines[int(experiment) + 1][2] = '{0:.3f}'.format(accuracy)
